...
export default function GranaryTab() {
  ...
  const I_RESTORE =
    "M13,3A9,9 0 0,0 4,12H1L4.89,15.89L4.96,16.03L9,12H6A7,7 0 0,1 13,5A7,7 0 0,1 20,12A7,7 0 0,1 13,19C11.07,19 9.32,18.2 8.06,16.94L6.64,18.36C8.27,20 10.5,21 13,21A9,9 0 0,0 22,12A9,9 0 0,0 13,3Z";
  const I_DELETE =
  ...
  async function performCreate() {
    const reason =
      newSiloReason().trim() || L_G().manualBackup || "Manual Backup";
    setIsCreating(true);
    try {
  ...
  return (
    <>
      ...
      >
        ...
        <div slot="content">
          {(L_G().dialogRestoreContent || "Restore {label}?").replace(
            "{label}",
            selectedSilo()?.label || "",
          )}
        </div>
      ...
      >
        ...
        <div slot="content">
          {(L_G().dialogDeleteContent || "Delete {label}?").replace(
            "{label}",
            selectedSilo()?.label || "",
          )}
        </div>
      ...
      >
        ...
        <div slot="actions">
          ...
          <md-text-button onClick={performCreate} disabled={isCreating()}>
            {isCreating()
              ? L_C().saving || "Saving..."
              : L_C().confirm || "Save"}
          </md-text-button>
                ...
                >
                  <div class="card-main">
                    <div class="type-icon-container">
                      <md-icon class="type-icon">
                        <svg viewBox="0 0 24 24">
                          <path d={isManual(silo.reason) ? I_MANUAL : I_AUTO} />
                        </svg>
                      </md-icon>
                    </div>
                  ...
                  <div class="card-actions">
                    ...
                    >
                      ...
                      {busyId() === silo.id
                        ? L_G().restoring || "Restoring..."
                        : L_G().restoreAction || "Restore"}
                    </md-filled-tonal-button>
                  </div>
                </div>
              )}
      ...
      <BottomActions>
        ...
        <md-filled-button onClick={openCreateDialog} disabled={isCreating()}>
          <md-icon slot="icon">
            <svg viewBox="0 0 24 24">
              <path d={I_ADD} />
            </svg>
